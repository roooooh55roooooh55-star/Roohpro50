...
const DynamicCaptions: React.FC<{ text: string, isActive: boolean, isShorts?: boolean }> = ({ text, isActive, isShorts = true }) => {
  ...
  // Define vibrant colors for the text glow
  const colors = [
  ...
  useEffect(() => {
    ...
    const showNextChunk = () => {
      ...
      // Pick a random neon color for each chunk
  ...
  const containerPosition = isShorts 
    ? "bottom-40 right-4 text-right items-end" 
    : "bottom-16 left-1/2 -translate-x-1/2 text-center items-center";

  return (
    <div className={`absolute z-[100] w-full max-w-[85%] pointer-events-none flex flex-col ${containerPosition}`}>
      <div 
      ...
      >
        {/* Minimalist container: No border, just subtle blur and text glow */}
        <div className="bg-black/30 backdrop-blur-[2px] px-4 py-2 rounded-xl">
             <span className={`text-lg md:text-xl font-black italic leading-relaxed tracking-wide ${colorClass}`}>
                {currentChunk}
             </span>
        </div>
      </div>
...
const ShortsPlayerOverlay: React.FC<ShortsPlayerOverlayProps> = ({ 
  ...
  const [playlist, setPlaylist] = useState<Video[]>(() => {
      ...
      return [initialVideo, ...shuffleArray(others)];
  });
  ...
  // Extend the playlist when we get close to the end
  useEffect(() => {
      if (currentIndex >= playlist.length - 3 && videoList.length > 1) {
          setPlaylist(prevPlaylist => {
              const newBatch = shuffleArray([...videoList]);
              const lastVideo = prevPlaylist[prevPlaylist.length - 1];
              if (newBatch[0].id === lastVideo.id && newBatch.length > 1) {
                  [newBatch[0], newBatch[1]] = [newBatch[1], newBatch[0]];
              }
              return [...prevPlaylist, ...newBatch];
          });
      }
  ...
  }, [currentIndex, playlist, isNarrativeOn]);
  ...
  // --- SMART PRELOADING LOGIC (NEXT VIDEO) ---
  useEffect(() => {
    const preloadNext = async () => {
        const nextIndices = [currentIndex + 1];
        for (const idx of nextIndices) {
            if (idx < playlist.length) {
                const video = playlist[idx];
                const url = formatVideoSource(video);
                try {
                    await fetch(url, { headers: { 'Range': 'bytes=0-3000000' }, mode: 'cors' });
                } catch (e) {}
            }
        }
    };
  ...
  useEffect(() => {
    (Object.values(videoRefs.current) as (HTMLVideoElement | null)[]).forEach(v => {
        if(v) {
            v.pause();
            if (v !== videoRefs.current[`main-${currentIndex}`]) {
                v.currentTime = 0; 
            }
        }
    });
  ...
  const handleTimeUpdate = (e: React.SyntheticEvent<HTMLVideoElement>, videoId: string) => {
      const video = e.currentTarget;
      const progress = video.currentTime / video.duration;
      onProgress(videoId, progress);

      if (video.duration - video.currentTime < 3) {
          const nextVid = videoRefs.current[`main-${currentIndex + 1}`];
      ...
      <div ref={containerRef} onScroll={handleScroll} className="flex-grow overflow-y-scroll snap-y snap-mandatory scrollbar-hide h-full w-full">
        {playlist.map((video, idx) => {
          if (Math.abs(idx - currentIndex) > 3) {
              return <div key={`${video.id}-${idx}`} className="h-full w-full snap-start bg-black"></div>;
          }
          ...
          const preloadStrategy = (idx === currentIndex || idx === currentIndex + 1) ? "auto" : "none";
            ...
            <div key={`${video.id}-${idx}`} className="h-full w-full snap-start relative bg-black flex overflow-hidden">
              ...
              >
                <video 
                    ...
                    className={`h-full w-full object-cover transition-opacity duration-500 contrast-110 saturate-125 opacity-100 landscape:object-contain bg-black`}
                    playsInline 
                    loop={false} 
                    crossOrigin="anonymous"
                    preload={preloadStrategy}
                    onEnded={() => isActive && handleNextVideo()} 
                    onTimeUpdate={(e) => isActive && handleTimeUpdate(e, video.id)}
                    onError={(e) => {
                ...
                <div className="video-fallback hidden absolute inset-0 flex items-center justify-center bg-neutral-900 z-10">
