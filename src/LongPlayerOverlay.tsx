...
const DynamicCaptions: React.FC<{ text: string, isActive: boolean }> = ({ text, isActive }) => {
    ...
    // Vibrant text colors instead of container borders
    const colors = [
    ...
    return (
      <div className="absolute bottom-16 left-1/2 -translate-x-1/2 z-[100] w-full max-w-[90%] pointer-events-none flex flex-col items-center justify-center text-center">
        ...
        >
           {/* Minimal container, glow on text */}
           <div className={`bg-black/30 backdrop-blur-[2px] px-6 py-3 rounded-2xl flex items-center justify-center gap-3 transition-colors duration-300`}>
             <span className={`text-xl md:text-2xl font-black italic leading-relaxed tracking-wide ${colorClass}`}>
                {currentChunk}
             </span>
           </div>
        </div>
