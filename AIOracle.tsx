...
const AIOracle: React.FC<AIOracleProps> = ({ onRefresh, allVideos = [], interactions, onPlayVideo }) => {
  ...
  const executeVideoPlay = (searchQuery?: string) => {
      ...
      if (targetVideo) {
          // Reset voice lockout if they watch a video? No, keep rules strict.
          // BUT, we close the AI overlay so they can see the video.
          // We DO NOT clear history.
          handleClose();
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (aiResponse.action === 'play_video') {
           // Small delay to let user read the "Okay, I'm taking you there" message
           setTimeout(() => {
      ...
      if (!isVoiceLocked) {
        ...
        if (speakableText) {
          ...
          // Check if limit reached (Now 7 instead of 5)
          if (newVoiceCount >= 7) {
              setIsVoiceLocked(true);
        ...
        // Increment text turns to determine when to show Suggestion Button
        setTextTurnCount(prev => prev + 1);

        // Keep text visible longer since no audio
        messageTimeoutRef.current = setTimeout(() => {
            setVisibleMessage(null);
        }, 8000);
      }
