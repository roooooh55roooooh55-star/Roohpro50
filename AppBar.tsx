...
const AppBar: React.FC<AppBarProps> = ({ onViewChange, onRefresh, currentView }) => {
  ...
  const getBtnClass = (view: AppView, colorClass: string, glowColor: string) => {
    ...
    // Reduced w-11 h-11 to w-9 h-9
    return `w-9 h-9 rounded-xl flex items-center justify-center transition-all duration-500 border active:scale-95 ${
      isActive 
  ...
  return (
    ...
    <header className="fixed top-0 left-0 right-0 h-16 bg-black/80 backdrop-blur-3xl z-[200] border-b border-white/5 px-3 flex items-center justify-between shadow-[0_15px_50px_rgba(0,0,0,0.9)]">
      ...
      >
        ...
        {/* Reduced Logo Size w-14 to w-11 */}
        <img src="https://i.top4top.io/p_3643ksmii1.jpg" className="w-11 h-11 rounded-full border border-red-500 relative z-10 shadow-[0_0_25px_red] group-hover:shadow-[0_0_50px_red] transition-all" alt="Logo" />
      </button>
      ...
      <div className="flex items-center gap-2">
        ...
        <button 
          ...
          className="w-9 h-9 rounded-xl bg-white/5 border border-white/20 flex items-center justify-center text-red-600 opacity-80 shadow-[0_0_15px_rgba(220,38,38,0.4)] hover:opacity-100 hover:shadow-[0_0_35px_rgba(220,38,38,0.8)] transition-all"
        >
